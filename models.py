...
class odoo-imppn(models.Model):
    _name = 'odoo-imppn.odoo-imppn'

    name = fields.Char()


  def alter_table_res_company():
      postgresql1 = "ALTER TABLE res_company ADD COLUMN teamsystem_id integer;"
      postgresql2 = "UPDATE res_company SET teamsystem_id = 5030 WHERE id = 1;"
      cnx_g = None
      try:
          # Connecting python postgresql database
          cnx = psycopg2.connect(host="http://192.168.1.10", port=5432, user="odoo", password="odoo", dbname="db")
  ...
  def select_form_account_invoice():
...
if __name__ == '__main__':
    if odoo-imppn().alter_table_res_company() == true :
       odoo-imppn().import_accounting()
     else : 
        print "altering table res_company wasn't succesfull ..."
